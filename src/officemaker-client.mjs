...
export async function getDocumentSchema(documentType, format = 'markdown') {
  const { tier, baseUrl } = getConfig();
  const params = new URLSearchParams({ documentType, format });
  const schemaPath = tier === 'paid' ? '/schema' : '/gpt/v1/schema';
  const response = await fetch(`${baseUrl}${schemaPath}?${params.toString()}`, {
    headers: buildHeaders({ accept: format === 'markdown' ? 'text/markdown' : 'application/json' })
  });
...
export async function createPreview({ documentKey, documentType, maxPreviewPages = 2 }) {
  const { tier } = getConfig();
  return requestJson(tier === 'paid' ? '/createPreview' : '/gpt/v1/create-preview', {
    method: 'POST',
    body:
      tier === 'paid'
        ? {
